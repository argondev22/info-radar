...
#### RSSソースを足す（設定だけ・Python不要）

`sources.yaml` に1エントリ足すだけ：
...
```yaml
  - name: AWS Security Blog        # ログ表示名
    ...
    section: セキュリティ           # 任意。同じ section が固まって上から順に並ぶ
```

#### RSSが無いページをスクレイプで足す

1. `src/collectors/` に**新しいファイルを1つ追加**（既存ファイルは触らない。`anthropic_news.py` が雛形）：
   ```python
   # src/collectors/my_site.py
   from bs4 import BeautifulSoup
   from ..models import Item
   from ..sources import Source
   from . import register
   from .common import http_get, clean_text
   ...
   def collect_my_site(src: Source) -> list[Item]:
       ...
       return items
   ```
2. `sources.yaml` に `kind: scrape_my_site` でソースを追加。

> `@register` を付けてファイルを置くだけで**起動時に自動登録**される。分岐やインポートの追記は不要。

### カテゴリを追加したい（例：株式）

**コードは実質1行 ＋ Notion でタグ作成だけ**（ID を調べる必要なし）：

1. `sources.yaml` にそのカテゴリのソースを追加：
   ```yaml
     - name: ロイター株式
       category: 株式
       kind: rss
...
on:
  schedule:
    - cron: "0 21 * * *"   # 06:00 JST（= 前日21:00 UTC）
```
...
- `sources.yaml`（リポジトリ直下）… **ソース定義。普段編集するのはここ**

```
src/
├─ sources.py     # sources.yaml のローダー（カテゴリ/section順を導出）
├─ collectors/    # 収集プラグイン（1 kind=1ファイル・@registerで自動登録）
